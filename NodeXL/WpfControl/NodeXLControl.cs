...
/// 
/// <para>
/// A tooltip can be displayed when the mouse hovers over an edge. The
/// tooltip can be simple text or a custom UIElement containing arbitrary
/// content. See <see cref="ShowEdgeToolTips" /> for details.
/// </para>
///
...
public partial class NodeXLControl : FrameworkElement
{
    ...
    public NodeXLControl()
    {
        ...
        // Create a helper object for displaying edge tooltips.

        CreateEdgeToolTipTracker();
        m_oEdgeToolTip = null;
    ...
    ~NodeXLControl()
    {
        ...
        if (m_oEdgeToolTipTracker != null)
        {
            m_oEdgeToolTipTracker.Dispose();
        }
    }
    #endif
    ...
    //*************************************************************************
    //  Property: ShowEdgeToolTips
    //
    /// <summary>
    /// Gets or sets a value indicating whether edge tooltips should be
    /// shown.
    /// </summary>
    ///
    /// <value>
    /// true to show edge tooltips.  The default value is false.
    /// </value>
    ///
    /// <remarks>
    /// An edge tooltip is a tootip that appears when the mouse is hovered
    /// over an edge.  Each edge has its own tooltip.
    ///
    /// <para>
    /// To use simple text for tooltips, set <see cref="ShowEdgeToolTips" />
    /// to true, then use Edge.<see cref="IMetadataProvider.SetValue" /> to
    /// assign a tooltip string to each of the graph's edges.  The key must
    /// be the reserved key ReservedMetadataKeys.<see
    /// cref="ReservedMetadataKeys.PerEdgeToolTip" /> and the value must be
    /// the tooltip string for the edge.
    /// </para>
    ///
    /// <para>
    /// To use a custom UIElement for tooltips instead of simple text, set <see
    /// cref="ShowEdgeToolTips" /> to true, then handle the <see
    /// cref="PreviewEdgeToolTipShown" /> event.  In your event handler, set
    /// the event argument's <see
    /// cref="EdgeToolTipShownEventArgs.EdgeToolTip" /> to a UIElement that
    /// you create.  You can use the event argument's <see
    /// cref="EdgeEventArgs.Edge" /> property to customize the UIElement
    /// based on which edge was hovered over.
    /// </para>
    ///
    /// <para>
    /// The <see cref="EdgeMouseHover" /> and <see cref="EdgeMouseLeave" />
    /// events fires regardless of whether edge tooltips are shown.
    /// </para>
    ///
    /// <para>
    /// Note that edge tooltips are entirely independent of the standard
    /// tooltip exposed by FrameworkElement.ToolTip.
    /// </para>
    ///
    /// </remarks>
    ///
    /// <seealso cref="PreviewEdgeToolTipShown" />
    /// <seealso cref="EdgeToolTipShownEventArgs" />
    //*************************************************************************
    ...
    ShowEdgeToolTips
    {
        ...
        set
        {
            ...
            if (!value)
            {
                ...
                ResetEdgeToolTipTracker();
            }

            m_bShowEdgeToolTips = value;
    ...
    //*************************************************************************
    //  Method: TryGetEdgeFromPoint()
    //
    /// <summary>
    /// Attempts to get the edge containing a specified <see cref="Point" />.
    /// </summary>
    ///
    /// <param name="point">
    /// Point to get a edge for.
    /// </param>
    ///
    /// <param name="edge">
    /// Where the <see cref="IEdge" /> object gets stored if true is
    /// returned.
    /// </param>
    ///
    /// <returns>
    /// true if a edge containing the point was found, false if not.
    /// </returns>
    ///
    /// <remarks>
    /// This method looks for a edge that contains <paramref name="point" />.
    /// If there is such a edge, the edge is stored at <paramref
    /// name="edge" /> and true is returned.  Otherwise, <paramref
    /// name="edge" /> is set to null and false is returned.
    ///
    /// <para>
    /// false is returned if an asynchronous drawing is in progress.  Check
    /// <see cref="IsLayingOutGraph" /> before calling this method.
    ...
    )
    {
        ...
        return (m_oGraphDrawer.TryGetEdgeFromPoint(point, out edge));
    }
    ...
    //
    /// <summary>
    /// Occurs when an edge is clicked.
    /// </summary>
    ///
    /// <remarks>
    /// In your event handler, do not change the selected state of the clicked
    /// edge.  That happens automatically.  An exception is thrown if you
    /// attempt to do this.
    /// </remarks>
    ///
    /// <seealso cref="MouseMode" />
    ...
    public event EdgeEventHandler EdgeClick;
    ...
    //
    /// <summary>
    /// Occurs when an edge is double-clicked.
    /// </summary>
    ///
    /// <seealso cref="MouseMode" />
    /// <seealso cref="MouseAlsoSelectsIncidentEdges" />
    ...
    public event EdgeEventHandler EdgeDoubleClick;


    //*************************************************************************
    //  Event: EdgeMouseHover
    //
    /// <summary>
    /// Occurs when the mouse pointer hovers over an edge.
    /// </summary>
    ///
    /// <remarks>
    /// This event occurs when the mouse pointer hovers over an edge.  If the
    /// mouse is moved to another edge, this event fires again.  If the mouse
    /// is left hovering over the edge for a predetermined period or is moved
    /// away from the edge, an <see cref="EdgeMouseLeave" /> event occurs.
    ///
    /// <para>
    /// If <see cref="ShowEdgeToolTips" /> is true, hovering the mouse over an
    /// edge causes the <see cref="EdgeMouseHover" /> event to fire,
    /// followed by <see cref="PreviewEdgeToolTipShown" />.  If <see
    /// cref="ShowEdgeToolTips" /> is false, only the <see
    /// cref="EdgeMouseHover" /> event fires.
    /// </para>
    ///
    /// </remarks>
    ///
    /// <seealso cref="EdgeMouseLeave" />
    //*************************************************************************

    [Category("Mouse")]

    public event EdgeEventHandler EdgeMouseHover;
    ...
    //
    /// <summary>
    /// Occurs when the mouse pointer is left hovering over an edge for a
    /// predetermined period or is moved away from all edges.
    /// </summary>
    ///
    /// <remarks>
    /// Hovering the mouse over an edge causes the <see
    /// cref="EdgeMouseHover" /> event to fire.  If the mouse is left
    /// hovering over the edge for a predetermined period or is moved away
    /// from the edge, the <see cref="EdgeMouseLeave" /> event fires.
    /// </remarks>
    ///
    /// <seealso cref="EdgeMouseHover" />
    //*************************************************************************

    [Category("Mouse")]

    public event EventHandler EdgeMouseLeave;


    //*************************************************************************
    //  Event: PreviewEdgeToolTipShown
    //
    /// <summary>
    /// Occurs when the mouse pointer hovers over an edge but before an edge
    /// tooltip is shown.
    /// </summary>
    ///
    /// <remarks>
    /// See <see cref="ShowEdgeToolTips" /> for information on how to show
    /// and customize edge tooltips.
    /// </remarks>
    //*************************************************************************

    [Category("Mouse")]

    public event EdgeToolTipShownEventHandler PreviewEdgeToolTipShown;
    ...
    //*************************************************************************
    //  Method: CreateEdgeToolTipTracker()
    //
    /// <summary>
    /// Creates a helper object for displaying edge tooltips and registers
    /// event handlers with it.
    /// </summary>
    //*************************************************************************
    ...
    CreateEdgeToolTipTracker()
    {
        m_oEdgeToolTipTracker = new WpfToolTipTracker();

        m_oEdgeToolTipTracker.HideDelayMs = EdgeToolTipHideDelayMs;

        m_oEdgeToolTipTracker.ShowToolTip +=
            new WpfToolTipTracker.ToolTipTrackerEvent(
                this.EdgeToolTipTracker_ShowToolTip);

        m_oEdgeToolTipTracker.HideToolTip +=
            new WpfToolTipTracker.ToolTipTrackerEvent(
                this.EdgeToolTipTracker_HideToolTip);
    }

    //*************************************************************************
    //  Method: CreateDefaultEdgeToolTip()
    //
    /// <summary>
    /// Creates a UIElement to use as a default edge tooltip.
    /// </summary>
    ///
    /// <param name="sToolTip">
    /// The tooltip string to use.  Can be empty but not null.
    ...
    //*************************************************************************
    //  Method: ResetEdgeToolTipTracker()
    //
    /// <summary>
    /// Removes any edge tooltip that might exist and resets the helper
    /// object that figures out when to show tooltips.
    /// </summary>
    //*************************************************************************
    ...
    ResetEdgeToolTipTracker()
    {
        ...
        RemoveEdgeToolTip();
        m_oEdgeToolTipTracker.Reset();
        m_oLastMouseMoveLocation = new Point(-1, -1);
    }

    //*************************************************************************
    //  Method: RemoveEdgeToolTip()
    //
    /// <summary>
    /// Removes any edge tooltip that might exist.
    /// </summary>
    //*************************************************************************
    ...
    RemoveEdgeToolTip()
    {
        ...
        if (m_oEdgeToolTip != null)
        {
            m_oGraphDrawer.RemoveVisualFromTopOfGraph(m_oEdgeToolTip);
            m_oEdgeToolTip = null;
        }
    }
    ...
    )
    {
        ...
        m_oGraphDrawer.TryGetEdgeFromPoint(oMouseLocation, out oEdge);

        // Update the tooltip tracker.

        m_oVertexToolTipTracker.OnMouseMoveOverObject(oVertex);
        m_oEdgeToolTipTracker.OnMouseMoveOverObject(oEdge);
    }
    ...
    //*************************************************************************
    //  Method: FireEdgeClick()
    //
    /// <summary>
    /// Fires the <see cref="EdgeClick" /> event if appropriate.
    /// </summary>
    ///
    /// <param name="oEdge">
    /// Clicked edge.  Can't be null.
    /// </param>
    //*************************************************************************
    ...
    )
    {
        ...
        FireEdgeEvent(this.EdgeClick, oEdge);
    }

    //*************************************************************************
    //  Method: FireEdgeDoubleClick()
    //
    /// <summary>
    /// Fires the <see cref="EdgeDoubleClick" /> event if appropriate.
    /// </summary>
    ///
    /// <param name="oEdge">
    /// Double-clicked edge.  Can't be null.
    /// </param>
    //*************************************************************************
    ...
    )
    {
        ...
        FireEdgeEvent(this.EdgeDoubleClick, oEdge);
    }

    //*************************************************************************
    //  Method: FireEdgeMouseHover()
    //
    /// <summary>
    /// Fires the <see cref="EdgeMouseHover" /> event if appropriate.
    /// </summary>
    ///
    /// <param name="oEdge">
    /// Hovered edge.  Can't be null.
    /// </param>
    //*************************************************************************
    ...
    )
    {
        ...
        FireEdgeEvent(this.EdgeMouseHover, oEdge);
    }

    //*************************************************************************
    //  Method: FireEdgeMouseLeave()
    //
    /// <summary>
    /// Fires the <see cref="EdgeMouseLeave" /> event if appropriate.
    /// </summary>
    //*************************************************************************
    ...
    FireEdgeMouseLeave()
    {
        ...
        EventUtil.FireEvent(this, this.EdgeMouseLeave);
    }
    ...
    //*************************************************************************
    //  Method: FirePreviewEdgeToolTipShown()
    //
    /// <summary>
    /// Fires the <see cref="PreviewEdgeToolTipShown" /> event if
    /// appropriate.
    /// </summary>
    ///
    /// <param name="oEdgeToolTipShownEventArgs">
    /// Event arguments.  The event handler may modify the <see
    /// cref="EdgeToolTipShownEventArgs.EdgeToolTip" /> property.
    /// </param>
    //*************************************************************************

    protected void
    FirePreviewEdgeToolTipShown
    (
        EdgeToolTipShownEventArgs oEdgeToolTipShownEventArgs
    )
    {
        Debug.Assert(oEdgeToolTipShownEventArgs != null);
        AssertValid();

        EdgeToolTipShownEventHandler oEdgeToolTipShownEventHandler =
            this.PreviewEdgeToolTipShown;

        if (oEdgeToolTipShownEventHandler != null)
        {
            oEdgeToolTipShownEventHandler(this,
                oEdgeToolTipShownEventArgs);
        }
    ...
    //*************************************************************************
    //  Method: FireEdgeEvent()
    //
    /// <summary>
    /// Fires an event with the signature EdgeEventHandler.
    /// </summary>
    ///
    /// <param name="oEdgeEventHandler">
    /// Event handler, or null if the event isn't being handled.
    /// </param>
    ///
    /// <param name="oEdge">
    ...
    FireEdgeEvent
    (
        EdgeEventHandler oEdgeEventHandler,
        IEdge oEdge
    )
    {
        Debug.Assert(oEdge != null);
        ...
        if (oEdgeEventHandler != null)
        {
            oEdgeEventHandler(this, new EdgeEventArgs(oEdge));
        }
    }
    ...
    )
    {
        ...
        if (m_oEdgeToolTip != null)
        {
            m_oEdgeToolTip.Measure(availableSize);
        }
    ...
    )
    {
        ...
        if (m_oEdgeToolTip != null)
        {
            ...
            m_oEdgeToolTip.Arrange(oBoundedToolTipRectangle);
        }
    ...
    )
    {
        ...
        // Remove any edge tooltip that might exist and reset the helper
        // object that figures out when to show tooltips.
    ...
    TryGetEdgeFromPoint(oMouseLocation, out oClickedEdge);
        ...
        if (bEdgeClicked)
        {
            ...
            if (e.ClickCount == 2)
            {
                FireEdgeDoubleClick(oClickedEdge);
            }
        }
    ...
    //*************************************************************************
    //  Method: EdgeToolTipTracker_ShowToolTip()
    //
    /// <summary>
    /// Handles the ShowToolTip event on the m_oEdgeToolTipTracker object.
    /// </summary>
    ///
    /// <param name="oSource">
    /// Standard event arguments.
    ...
    private void
    EdgeToolTipTracker_ShowToolTip
    (
        Object oSource,
        ToolTipTrackerEventArgs oToolTipTrackerEventArgs
    )
    {
        ...
        Debug.Assert(oToolTipTrackerEventArgs.Object is IEdge);

        IEdge oEdge = (IEdge)oToolTipTrackerEventArgs.Object;

        // Fire a EdgeMouseHover event in case the application wants to take
        // additional action.

        FireEdgeMouseHover(oEdge);

        if (!m_bShowEdgeToolTips)
        {
            // Edge tooltips aren't being shown, so no additional action is
            // required.

            return;
        }

        // Give PreviewEdgeToolTipShown event handlers an opportunity to
        // override the default tooltip.

        EdgeToolTipShownEventArgs oEdgeToolTipShownEventArgs =
            new EdgeToolTipShownEventArgs(oEdge);

        FirePreviewEdgeToolTipShown(oEdgeToolTipShownEventArgs);

        m_oEdgeToolTip = oEdgeToolTipShownEventArgs.EdgeToolTip;
        ...
        if (m_oEdgeToolTip == null)
        {
            ...
            Object oEdgeToolTipAsObject;

            if (!oEdge.TryGetValue(ReservedMetadataKeys.PerEdgeToolTip,
                typeof(String), out oEdgeToolTipAsObject))
            {
                // No.  Nothing needs to be done.
            ...
            m_oEdgeToolTip = CreateDefaultEdgeToolTip(
                (String)oEdgeToolTipAsObject);
        }
        ...
        Double dScale = 1.0 / this.ScaleTransformForRender.ScaleX;
        m_oEdgeToolTip.RenderTransform = new ScaleTransform(dScale, dScale);

        m_oGraphDrawer.AddVisualOnTopOfGraph(m_oEdgeToolTip);
    ...
    //*************************************************************************
    //  Method: EdgeToolTipTracker_HideToolTip()
    //
    /// <summary>
    /// Handles the HideToolTip event on the m_oEdgeToolTipTracker object.
    /// </summary>
    ///
    /// <param name="oSource">
    /// Standard event arguments.
    ...
    private void
    EdgeToolTipTracker_HideToolTip
    (
        Object oSource,
        ToolTipTrackerEventArgs oToolTipTrackerEventArgs
    )
    ...
    AssertValid()
    {
        ...
        // m_oVertexToolTip
        Debug.Assert(m_oEdgeToolTipTracker != null);
        // m_oEdgeToolTip
    ...
    /// Number of milliseconds to wait before hiding an edge tooltip.

    protected const Int32 EdgeToolTipHideDelayMs = 60 * 60 * 1000;
    ...
    /// true to show edge tooltips.

    protected Boolean m_bShowEdgeToolTips;
    ...
    /// Helper object for figuring out when to show edge tooltips.

    private WpfToolTipTracker m_oEdgeToolTipTracker;
    ...
    /// The edge tooltip being displayed, or null if no edge tooltip is
    /// being displayed.
