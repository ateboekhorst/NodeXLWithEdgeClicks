...
public class GraphDrawer : DrawerBase
{
    ...
    ///
    /// <returns>
    /// true if an edge containing the point was found, false if not.
    /// </returns>
    ///
    /// <remarks>
    /// This method looks for an edge that contains <paramref name="point" />.
    /// If there is such an edge, the edge is stored at <paramref
    ...
    )
    {
        ...
        if (m_oSelectedEdgeDrawingVisuals == null || m_oUnselectedEdgeDrawingVisuals == null)
        {
            // The graph hasn't been drawn yet.
        ...
        // The edge are represented by DrawingVisual child objects of
        // m_oSelectedEdgeDrawingVisuals and m_oUnselectedEdgeDrawingVisuals

        HitTestResult oHitTestResult =
            m_oSelectedEdgeDrawingVisuals.HitTest(point) ?? m_oUnselectedEdgeDrawingVisuals.HitTest(point);
        ...
        if (oHitTestResult != null)
        {
            ...
            if (typeof(DrawingVisual).IsInstanceOfType(oVisualHit))
            {
                ...
                edge = RetrieveEdgeFromDrawingVisual(
                    (DrawingVisual)oVisualHit);
    ...
    )
    {
            ...
            out oEdgeDrawingHistory) )
        {
            ...
            DrawingVisual oEdgeChildDrawingVisual =
                oEdgeDrawingHistory.DrawingVisual;

            GetEdgeDrawingVisuals(oEdgeDrawingHistory).Children.Add(
                oEdgeChildDrawingVisual);
            ...
            SaveEdgeOnDrawingVisual(oEdge, oEdgeChildDrawingVisual);
        }
    }
    ...
    //*************************************************************************
    //  Method: SaveEdgeOnDrawingVisual()
    //
    /// <summary>
    /// Saves an edge on the DrawingVisual with which the edge was drawn.
    /// </summary>
    ///
    /// <param name="oEdge">
    /// The vertex that was drawn.
    ...
    ///
    /// <param name="oDrawingVisual">
    /// The DrawingVisual with which <paramref name="oEdge" /> was drawn.
    /// </param>
    ///
    /// <remarks>
    /// The edge can be retrieved from the DrawingVisual with <see
    /// cref="RetrieveEdgeFromDrawingVisual" />.
    /// </remarks>
    //*************************************************************************

    protected void
    SaveEdgeOnDrawingVisual
    (
        IEdge oEdge,
        DrawingVisual oDrawingVisual
    )
    {
        ...
        oDrawingVisual.SetValue(FrameworkElement.TagProperty, oEdge);
    }
    ...
    //*************************************************************************
    //  Method: RetrieveEdgeFromDrawingVisual()
    //
    /// <summary>
    /// Retrieves an edge from the DrawingVisual with which the edge was
    /// drawn.
    /// </summary>
    ///
    /// <param name="oDrawingVisual">
    /// The DrawingVisual to retrieve an edge from.
    /// </param>
    ///
    /// <returns>
    /// The edge that was drawn with <paramref name="oDrawingVisual" />.
    /// </returns>
    ///
    /// <remarks>
    /// This method retrieves the edge that was saved on a DrawingVisual by
    /// <see cref="SaveEdgeOnDrawingVisual" />.
    /// </remarks>
    //*************************************************************************

    protected IEdge
    RetrieveEdgeFromDrawingVisual
    (
        DrawingVisual oDrawingVisual
    )
    {
        ...
        Object oEdgeAsObject =
            oDrawingVisual.GetValue(FrameworkElement.TagProperty);

        Debug.Assert(oEdgeAsObject is IEdge);

        return ((IEdge)oEdgeAsObject);
    }
